...
void cmdq_sec_mtee_setup_context(struct cmdq_sec_mtee_context *tee)
{
	...
	strncpy(tee->ta_uuid, ta_uuid, sizeof(tee->ta_uuid) - 1);
	tee->ta_uuid[sizeof(tee->ta_uuid) - 1] = '\0';
	strncpy(tee->wsm_uuid, wsm_uuid, sizeof(tee->wsm_uuid) - 1);
	tee->wsm_uuid[sizeof(tee->wsm_uuid) - 1] = '\0';
