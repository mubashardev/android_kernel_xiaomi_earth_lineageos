...
void qvga_i2c4_mclk(void)
{
	...
	if(g_gc6133c == NULL || g_gc6133c->gc6133c_pinctrl == NULL ||gc6133c_mclk_on == NULL )
	{
		return;
	}
	ret = pinctrl_select_state(g_gc6133c->gc6133c_pinctrl, gc6133c_mclk_on);
	if (ret < 0) {
		qvga_dev_err(g_gc6133c->dev, "%s: select failed for mclk on\n", __func__);
	}
}
/* code at 2022/9/6 end */
...
* gc6133c i2c driver
*****************************************************************************/
static int gc6133c_i2c4_probe(struct i2c_client *client,
...
static struct i2c_driver gc6133c_i2c_driver = {
	.driver = {
		   .owner = THIS_MODULE,
		   .name =  "gc6133c_yuv_i2c4",
		   .of_match_table = gc6133c_of_match,
		   },
	.probe = gc6133c_i2c4_probe,
